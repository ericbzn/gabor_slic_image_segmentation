    ...
    for features_input_file in input_files:
        with h5py.File(hdf5_indir_feat / features_input_file, "r+") as features_file:
            ...
            for im_file, img, g_energies in zip(img_ids, images, gabor_features_norm):
                ...
                ''' THIRD METHOD: Spectral Clustering on complete RAG '''
                # Third method parameters
                sigma_method = 'global'  # Choose: 'global' or 'local'
                graph_spec, regions_spec, aff_matrix = spectral_clustering(graph_weighted, weights, sigma_method, regions)

                ''' FORTH METHOD: Spectral Clustering on MST RAG '''
                # Forth method parameters
                graph_mst_spec, regions_mst_spec, aff_matrix_mst = spectral_clustering(graph_mst, weights, sigma_method, regions)
                ...
                outdir = 'outdir/' + num_imgs_dir + 'spectral_clustering/' + method + '/graph_' + graph_type + '/' + features_input_file[:-3] + '/computation_support/'
                ...
                if sigma_method == 'global':
                    outdir = 'outdir/' + num_imgs_dir + 'spectral_clustering/' + method + '/graph_' + graph_type + '/' + features_input_file[:-3] + '/global_sigma/results/'
                elif sigma_method == 'local':
                    outdir = 'outdir/' + num_imgs_dir + 'spectral_clustering/' + method + '/graph_' + graph_type + '/' + features_input_file[:-3] + '/local_sigma/results/'
                ...
                show_and_save_result(img, regions_spec, fig_title, img_name, fontsize, save_fig, outdir, file_name)
                ...
                if sigma_method == 'global':
                    outdir = 'outdir/' + num_imgs_dir + 'spectral_clustering/' + method + '/graph_' + graph_type + '/' + features_input_file[:-3] + '/global_sigma/results_mst/'

                elif sigma_method == 'local':
                    outdir = 'outdir/' + num_imgs_dir + 'spectral_clustering/' + method + '/graph_' + graph_type + '/' + features_input_file[:-3] + '/local_sigma/results_mst/'


                if not os.path.exists(outdir):
                    os.makedirs(outdir)

                # fig_title = 'Spectral Graph (MST)'
                # img_name = '_spec_mst_graph'
                # show_and_save_spectralgraph(graph_mst, fig_title, img_name, fontsize, save_fig, outdir, file_name)
                ...
                fig_title = 'Spectral Clustering Result (MST)'
                img_name = '_mst_spec_result'
                show_and_save_result(img, regions_mst_spec, fig_title, img_name, fontsize, save_fig, outdir, file_name)
